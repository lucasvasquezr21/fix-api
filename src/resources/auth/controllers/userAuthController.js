...
export const registerUser = async (req, res) => {
  try {
    ...
    const accessToken = jwt.sign({ userId: newUser._id }, environment.secretKey, { expiresIn: '50m' });
...
export const loginUser = async (req, res) => {
  try {
    ...
    // Generar un token de acceso
    const accessToken = jwt.sign({ userId: user._id }, environment.secretKey, { expiresIn: '50m' });
